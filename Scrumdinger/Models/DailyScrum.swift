...
extension DailyScrum {
    ...
    static func createNewScrum(from data: Data) -> DailyScrum {
        return DailyScrum(title: data.title, attendees: data.attendees, lengthInMinutes: Int(data.lengthInMinutes), color: data.color)
    }
}
